...
export function Stream() {
  ...
  }, [channelParameters.cameras, channelParameters.mics, cameraIdx, micIdx, isScreenShare])
  ...
  const startStream = async () => {
    try {
      ...
      let cameraStream = !isScreenShare ? await navigator.mediaDevices.getUserMedia({
        video: { deviceId: channelParameters.cameras[cameraIdx].deviceId, width: { ideal: '1920' }, height: { ideal: '1080' } }
      }) :
      ...
      let micStream = await navigator.mediaDevices.getUserMedia({
        audio: { deviceId: channelParameters.mics[micIdx].deviceId },
      })
      ...
      client.startBroadcast(channelParameters.streamKey)
      setStatus('live')
    }
    catch (err) {
      console.log(err)
  ...
  const endEvent = async () => {
    try {
      await eventService.endEvent(event._id, event.fund ? true : isShare)
      setStatus('ended')
      setModal(false)
      ...
      {!isMobile && <div className="stream-container">
        ...
        <div className="stream">
          <div className="stream-video">
            {status === 'no-devices' && <div className="no-camera">
              ...
              <p>Alternatively, you can <span className="main-color clickable" onClick={setIsScreenShare(true)}>Share your screen</span>.</p>
            </div>}
            {status !== 'no-devices' && <video ref={localVideoRef} autoPlay />}
          </div>
        ...
        <StreamChat eventName={event._id} end={status === 'ended'} cameraIdx={cameraIdx} cameras={channelParameters.cameras} />
      </div>}
      {modal && <>
