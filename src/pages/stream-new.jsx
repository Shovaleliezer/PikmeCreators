...
export function Stream() {
    ...
    useEffect(() => {
        ...
        play()
    }, [cameraIdx, micIdx, isScreenShare, isMuted, volume, mode])
    ...
    const startStream = async () => {
        await client.publish([channelParameters.videoStream, channelParameters.audioStream])
        ...
        try {
            ...
            const config = {
                ...
                encoderConfig: {
                    ...
                    height: { max: 1080, min: 360 },
                }
            }
            if (isMobile) config.facingMode = mode
            else config.cameraId = channelParameters.cameras[cameraIdx].deviceId
            const video = await AgoraRTC.createCameraVideoTrack(config)
            return video
        }
        catch (err) {
    ...
    const switchCamera = async () => {
        if (mode === 'user') setMode('environment')
        else setMode('user')
    }
    ...
    const handleModal = async () => {
        if (modal === 'end') {
            ...
            try {
                const confirm = await adminService.startShow(event._id)
                if (confirm) startStream()
        ...
        return (<>
            {!isMobile && <div className="stream-container">
                <div className="settings noselect">
                    ...
                    {openOpt === 'camera' && <>
                        {channelParameters.cameras.map((camera, idx) => <div key={idx} className={(!isScreenShare && cameraIdx === idx) ? 'sub sec-color back-stream' : 'sub'} onClick={() => onCameraClick(idx)}><p>{camera.label}</p></div>)}
                ...
                <div className="stream">
                    <div className="stream-video">
                        ...
                        <div id="agora_local" className="agora-local-pc" style={{ zIndex: status !== 'noDevices' ? 1 : -1 }} />
                    </div>
            ...
            {isMobile && <section className="stream-mobile" >
                <section className="left-wrapper">
                    ...
                    <div className="lower" >
                        ...
                        <img src={require('../style/imgs/stream/switch-camera.png')} onClick={() => setMode('user')} />
                        <img src={require('../style/imgs/stream/switch-camera.png')} onClick={() => setMode('environment')} />
                        <img src={require('../style/imgs/stream/switch-camera.png')} onClick={() => clearAll()} />
                    </div>
                </section>
